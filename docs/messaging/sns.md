...
AWS SNS is a publish/fan-out system, so it defines a single producer `send`
(publish) span, extending the [common messaging attributes](messaging-spans.md#messaging-attributes)
with the SNS-specific attributes below.

<!-- weaver .registry.spans[] | select(.type == "messaging.aws.sns.send.producer") -->
...
Describes an AWS SNS `send` (publish) operation.
