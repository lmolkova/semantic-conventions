...
| [`gen_ai.tool.call.result`](/docs/registry/attributes/gen-ai.md) | any | The result returned by the tool call (if any and if execution was successful). [2] | `{"temperature_range": {"high": 75, "low": 60}, "conditions": "sunny"}` | `Conditionally Required` opt_in | ![Development](https://img.shields.io/badge/-development-blue) |
| [`gen_ai.tool.call.arguments`](/docs/registry/attributes/gen-ai.md) | any | Parameters passed to the tool call. [3] | `{"location": "San Francisco?", "date": "2025-10-01"}` | `Recommended` | ![Development](https://img.shields.io/badge/-development-blue) |
...
| [`gen_ai.tool.type`](/docs/registry/attributes/gen-ai.md) | string | Type of the tool utilized by the agent [4] | `function`; `extension`; `datastore` | `Recommended` | ![Development](https://img.shields.io/badge/-development-blue) |
| [`gen_ai.tool.version`](/docs/registry/attributes/gen-ai.md) | string | Version of the tool utilized by the agent. [5] | `0.1.2` | `Recommended` if available. TODO: is it ever available? | ![Development](https://img.shields.io/badge/-development-blue) |

**[1] `error.type`:** The `error.type` SHOULD match the error code returned by the Generative AI provider or the client library,
the canonical name of exception that occurred, or another low-cardinality error identifier.
Instrumentations SHOULD document the list of errors they report.

**[2] `gen_ai.tool.call.result`:** TODO: we need to decide what to do with well-known function call results
...
**[3] `gen_ai.tool.call.arguments`:** TODO: we need to decide what to do with well-known function call params (eg. filesearch) - do we want to define a format and normalize across models/frameworks or leave them as is and record a complex object.
...
**[5] `gen_ai.tool.version`:** TODO: where are we going to get this from? None of the models have it
