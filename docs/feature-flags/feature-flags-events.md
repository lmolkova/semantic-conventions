...
Feature flag evaluations SHOULD be recorded as attributes on the [Event](https://github.com/open-telemetry/opentelemetry-specification/blob/v1.58.0/specification/logs/data-model.md#events) passed to the [Logger](https://github.com/open-telemetry/opentelemetry-specification/blob/v1.58.0/specification/logs/api.md#logger) emit operations.

> [!NOTE]
> Events are in-development and not yet available in some languages. Check the [spec-compliance matrix](https://github.com/open-telemetry/opentelemetry-specification/blob/v1.58.0/spec-compliance-matrix.md#logs) to see the implementation status in the corresponding language.

## Evaluation event

The table below indicates which attributes should be added to the [Event](https://github.com/open-telemetry/opentelemetry-specification/blob/v1.58.0/specification/logs/data-model.md#log-and-event-record-definition) and their types.

<!-- weaver .registry.events[] | select(.name == "feature_flag.evaluation") -->
