...
<!-- weaver .refinements.spans[] | select(.id == "aws.lambda.server") -->
...
parsed from the value of the attribute using the [AWS X-Ray Propagator](https://github.com/open-telemetry/opentelemetry-specification/blob/v1.58.0/specification/context/api-propagators.md) and
