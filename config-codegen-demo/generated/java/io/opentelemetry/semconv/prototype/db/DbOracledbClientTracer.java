...
public final class DbOracledbClientTracer {
  ...
  private DbOracledbClientTracer(
  ...
  public static DbOracledbClientTracer create(
      Tracer tracer, ConfigProvider configProvider) {
    DbOracledbClientTracer result =
        new DbOracledbClientTracer(tracer, configProvider);
  ...
  public DbOracledbClientSpan start(
      String spanName,
      String dbNamespace,
      String dbQuerySummary,
      String dbQueryText,
      String oracleDbDomain,
      String oracleDbInstanceName,
      String oracleDbName,
      String oracleDbPdb,
    ...
    if (!isEnabled(state)) {
      return DbOracledbClientSpan.noop();
    }
    ...
    if (oracleDbDomain != null) {
      attributes.put(DbAttributes.ORACLE_DB_DOMAIN, oracleDbDomain);
    }
    if (oracleDbInstanceName != null) {
      attributes.put(DbAttributes.ORACLE_DB_INSTANCE_NAME, oracleDbInstanceName);
    }
    if (oracleDbName != null) {
      attributes.put(DbAttributes.ORACLE_DB_NAME, oracleDbName);
    }
    if (oracleDbPdb != null) {
      attributes.put(DbAttributes.ORACLE_DB_PDB, oracleDbPdb);
    }
    if (oracleDbService != null) {
      attributes.put(DbAttributes.ORACLE_DB_SERVICE, oracleDbService);
    }
    ...
    Span span =
        tracer
            ...
            .startSpan();
    return new DbOracledbClientSpan(
        span,
